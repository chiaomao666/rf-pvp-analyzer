...
export default function Home() {
  ...
  const emptyAction = !profile ? <Link href="/account"><Button className="blueprint-button primary-button">前往帳號工作區</Button></Link> : <div className="empty-actions"><Link href="/record"><Button className="blueprint-button primary-button"><Plus size={16} />新增第一場</Button></Link><Link href="/import"><Button variant="outline" className="blueprint-button secondary-button"><Upload size={16} />匯入 JSON</Button></Link></div>;
  ...
  return <div className="page-enter">
    <section className="page-titlebar">
      <div>
        ...
        <p>{workspaceName ? `正在檢視 ${workspaceName} 的本機戰績；請定期下載帳號專屬 JSON 備份。` : "尚未選取帳號工作區；此頁不會顯示或建立未歸屬戰績。"}</p>
      </div>
    ...
    {!loading && data?.total === 0 ? <EmptyData title={profile ? "此帳號工作區尚未建立第一筆戰績" : "請先選取帳號工作區"} description={profile ? "可手動新增單場資料，或貼上 PVP 守衛腳本匯出的 JSON。資料只保存在本瀏覽器的此帳號工作區，請定期下載備份。" : "登入確認遊戲帳號、選取既有本機工作區，或使用示範模式後，才能建立及匯入戰績。"} action={emptyAction} /> : <section className="overview-grid">
