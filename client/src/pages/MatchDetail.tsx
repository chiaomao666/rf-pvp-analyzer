...
export default function MatchDetail({ id }: { id: number }) {
  ...
  const opponentUnion = extractIdentity(data.rawPayload, ["opponent_union_name", "opponentUnionName", "opponent_guild_name", "opponentGuildName", "opponent_organization_name"]);
  const ownPlayer = ownProfile?.playerName || "目前工作區玩家";
  const ownUnion = ownProfile?.unionName || "未提供組織名稱";
