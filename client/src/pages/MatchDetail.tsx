...
function extractIdentity(payload: unknown, keys: string[]): string | undefined {
  ...
  for (const key of keys) if ((typeof object[key] === "string" || typeof object[key] === "number") && String(object[key]).trim()) return String(object[key]).trim();
  for (const value of Object.values(object)) { const found = extractIdentity(value, keys); if (found) return found; }
  return undefined;
}

function formatIdentity(union: string, playerId?: string) {
  return playerId ? `${union} (玩家ID: ${playerId})` : union;
}
