...
export default function Matches() {
  ...
  return <div className="page-enter">
    ...
    {matches.isLoading ? <div className="loading-block">讀取戰績資料…</div> : !matches.data?.length ? <EmptyData title="找不到符合條件的戰績" description="調整日期、模式或結果篩選，或新增一筆排名戰紀錄。" action={<Link href="/record"><Button className="blueprint-button primary-button"><Plus size={16} />新增對戰</Button></Link>} /> : <section className="history-table technical-frame"><div className="history-head"><span>日期／模式</span><span>結果</span><span>我的隊伍</span><span>對手隊伍</span><span>排名變動</span><span /></div>{matches.data.map(match => <Link href={`/matches/${match.id}`} key={match.id} className="history-row"><div><time>{formatLocalDateTime(match.battleAt)}</time><ModeBadge mode={match.mode} /></div><OutcomeBadge outcome={match.outcome} /><TeamStrip members={match.playerTeam as TeamMember[]} /><TeamStrip members={match.opponentTeam as TeamMember[]} tone="opponent" /><RankDelta before={match.rankBefore} after={match.rankAfter} /><span className="row-arrow">→</span></Link>)}</section>}
  </div>;
}
