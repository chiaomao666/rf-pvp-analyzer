...
import { checkLocalBridge, getLocalBridgeCursor, isLocalBridgeEnabled, LocalBridgeStatus, pollLocalBridge, setLocalBridgeCursor, setLocalBridgeEnabled } from "@/lib/localBridge";
...
export default function Account() {
  ...
  }, [bridgeEnabled, session?.profile.id]);
  ...
  const toggleBridge = () => { const next = !bridgeEnabled; setBridgeEnabled(next); setLocalBridgeEnabled(next); if (!next) { setBridgeLastError(""); setStatus({ tone: "info", text: "已關閉本機 bridge 輪詢；網站不會再讀取 localhost。" }); } else setStatus({ tone: "info", text: "已啟用本機 bridge 輪詢。請先在電腦上執行 `pnpm bridge`，再由獲准的 mod POST 戰績摘要。" }); };
  ...
  return <div className="account-page">
    ...
    <section className="panel bridge-panel"><header><span><Link2 size={17} /></span><div><p className="eyebrow">LOCALHOST BRIDGE / PASSIVE CAPTURE</p><h2>本機橋接</h2><p>網站只輪詢 `127.0.0.1` 的記憶體佇列；bridge 不會接收密碼、token、cookie 或任意原始封包，只接受由獲准 mod 主動送出的最小戰績摘要。</p></div><div className={`bridge-indicator ${bridgeStatus}`}><i /><b>{bridgeLabel(bridgeStatus)}</b></div></header><div className="bridge-controls"><div><strong>{bridgeStatus === "online" ? "已連線到本機服務" : bridgeEnabled ? "等待本機服務" : "目前未啟用"}</strong><small>{bridgeLastError || "資料只會匯入目前工作區；切換工作區不會重新驗證官方帳號。"}</small></div><button type="button" className={bridgeEnabled ? "secondary-action bridge-toggle active" : "secondary-action bridge-toggle"} onClick={toggleBridge}><Wifi size={15} />{bridgeEnabled ? "關閉 bridge 輪詢" : "啟用 bridge 輪詢"}</button></div><div className="bridge-stats"><span>QUEUE <b>{bridgeQueue}</b></span><span>本頁匯入 <b>{bridgeImported}</b></span><span>游標 <b>{getLocalBridgeCursor()}</b></span></div><p className="bridge-note">安全邊界：bridge 綁定 loopback、只存在記憶體，重啟後佇列會清空。若網站是 GitHub Pages，bridge 仍必須在同一台電腦上另行啟動。</p></section>
