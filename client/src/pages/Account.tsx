...
export default function Account() {
  ...
  const onSiteLogout = async () => { await logoutRemoteSite(); setSitePassword(""); setSiteAdminPassword(""); setCurrentSitePassword(""); setNewSitePassword(""); setConfirmSitePassword(""); setSiteSessionActive(false); setBridgeStatus("offline"); setStatus({ tone: "info", text: "已登出網站的讀取與管理 session；本機戰績與工作區未刪除。mod 仍可使用獨立的 PVP_WRITE_SECRET 送出心跳與戰績。" }); };
  const onChangeSitePassword = async () => {
    if (!siteAdminPassword || !currentSitePassword || !newSitePassword || !confirmSitePassword) { setStatus({ tone: "error", text: "請完整填寫管理者密碼、目前網站密碼與新網站密碼。" }); return; }
    if (newSitePassword !== confirmSitePassword) { setStatus({ tone: "error", text: "兩次輸入的新網站密碼不一致。" }); return; }
    if (newSitePassword.length < 12 || newSitePassword.length > 256) { setStatus({ tone: "error", text: "新網站密碼必須介於 12 至 256 個字元。" }); return; }
    setBusy(true); setStatus(null);
    try {
      await changeRemoteSitePassword({ adminPassword: siteAdminPassword, currentPassword: currentSitePassword, newPassword: newSitePassword });
      setSiteAdminPassword(""); setCurrentSitePassword(""); setNewSitePassword(""); setConfirmSitePassword(""); setSiteSessionActive(false); setBridgeStatus("offline");
      setStatus({ tone: "success", text: "網站密碼已變更；為安全起見，所有既有網站 session 都已失效。請以新密碼重新登入。" });
    } catch (error) { setStatus({ tone: "error", text: error instanceof Error ? error.message : "網站密碼變更失敗。" }); }
    finally { setBusy(false); }
  };
  ...
  return <div className="account-page">
    ...
    <section className="panel bridge-panel"><header><span><Link2 size={17} /></span><div><p className="eyebrow">{bridgeMode === "remote" ? "REMOTE BACKEND / PASSIVE CAPTURE" : "LOCALHOST BRIDGE / PASSIVE CAPTURE"}</p><h2>{bridgeMode === "remote" ? "網站後端接收" : "本機橋接"}</h2><p>{bridgeMode === "remote" ? "網站後端只接受由獲准 mod 主動送出的最小戰績摘要，依 workspaceId 分區並做來源去重；目前試作佇列保存在記憶體。" : "網站只輪詢 `127.0.0.1` 的記憶體佇列；bridge 不會接收密碼、token、cookie 或任意原始封包，只接受由獲准 mod 主動送出的最小戰績摘要。"}</p></div><div className={`bridge-indicator ${bridgeStatus}`}><i /><b>{bridgeLabel(bridgeStatus)}</b></div></header><div className="bridge-controls"><div><strong>{bridgeStatus === "online" ? (bridgeMode === "remote" ? "已連線到網站後端" : "已連線到本機服務") : bridgeEnabled ? (bridgeMode === "remote" ? "等待網站後端" : "等待本機服務") : "目前未啟用"}</strong><small>{bridgeLastError || "資料只會匯入目前工作區；切換工作區不會重新驗證官方帳號。"}</small></div><div className="bridge-mode-switch"><button type="button" className={bridgeMode === "local" ? "active" : ""} onClick={() => switchBridgeMode("local")}>本機</button><button type="button" className={bridgeMode === "remote" ? "active" : ""} onClick={() => switchBridgeMode("remote")}>網站後端</button></div><button type="button" className={bridgeEnabled ? "secondary-action bridge-toggle active" : "secondary-action bridge-toggle"} onClick={toggleBridge}><Wifi size={15} />{bridgeEnabled ? "關閉 bridge 輪詢" : "啟用 bridge 輪詢"}</button></div>{bridgeMode === "remote" && <div className="remote-key-box"><label><span>網站密碼</span><input type="password" autoComplete="current-password" value={sitePassword} onChange={event => setSitePassword(event.target.value)} placeholder="輸入網站管理者設定的密碼" disabled={busy || siteSessionActive} /></label><div className="bridge-key-actions"><small>{siteSessionActive ? "網站 session 已建立；此 session 只控制網站讀取與管理，密碼不會寫入瀏覽器儲存。" : "登入後由 Worker 發出 HttpOnly session cookie；登出網站不會停止 mod 的獨立心跳或戰績上傳。"}</small>{siteSessionActive ? <button type="button" className="secondary-action" onClick={() => void onSiteLogout()} disabled={busy}>登出網站</button> : <button type="button" className="secondary-action" onClick={() => void onSiteLogin()} disabled={busy}>登入網站</button>}</div>{siteSessionActive && <section className="site-password-admin" aria-labelledby="site-password-admin-title"><header><span><KeyRound size={16} /></span><div><p className="eyebrow">OWNER CONTROL / SESSION REQUIRED</p><h3 id="site-password-admin-title">變更網站登入密碼</h3><p>需同時驗證目前網站密碼與只有管理者知道的管理者密碼。變更後會登出所有網站 session。</p></div></header><form className="site-password-admin-form" onSubmit={event => { event.preventDefault(); void onChangeSitePassword(); }}><label><span>管理者密碼</span><input type="password" autoComplete="off" value={siteAdminPassword} onChange={event => setSiteAdminPassword(event.target.value)} placeholder="僅供本次變更驗證" disabled={busy} /></label><label><span>目前網站密碼</span><input type="password" autoComplete="current-password" value={currentSitePassword} onChange={event => setCurrentSitePassword(event.target.value)} placeholder="輸入目前登入密碼" disabled={busy} /></label><label><span>新網站密碼</span><input type="password" autoComplete="new-password" value={newSitePassword} onChange={event => setNewSitePassword(event.target.value)} placeholder="12 至 256 個字元" disabled={busy} /></label><label><span>確認新網站密碼</span><input type="password" autoComplete="new-password" value={confirmSitePassword} onChange={event => setConfirmSitePassword(event.target.value)} placeholder="再次輸入新密碼" disabled={busy} /></label><small>密碼只會隨本次 HTTPS 請求送到 Worker，不會寫入 GitHub、IndexedDB 或 localStorage。`PVP_WRITE_SECRET` 不會出現在此頁。</small><button type="submit" className="primary-action" disabled={busy}>{busy ? "變更中…" : "變更網站密碼並登出所有 session"}</button></form></section>}</div>}<div className="bridge-stats"><span>QUEUE <b>{bridgeQueue}</b></span><span>本頁匯入 <b>{bridgeImported}</b></span><span>游標 <b>{getLocalBridgeCursor()}</b></span></div><p className="bridge-note">安全邊界：{bridgeMode === "remote" ? "網站登入只控制瀏覽器讀取與管理；mod 以獨立 PVP_WRITE_SECRET 傳送心跳與戰績，網站登出不會撤銷該寫入授權。後端只接受白名單摘要並依 workspaceId 分區。" : "bridge 綁定 loopback、只存在記憶體，重啟後佇列會清空。若網站是 GitHub Pages，bridge 仍必須在同一台電腦上另行啟動。"}</p></section>
