import { activateStoredWorkspace, createDemoWorkspace, getWorkspaceSession, loginOfficialAccount, logoutWorkspace, OfficialLoginError, restoreStoredWorkspace } from "@/lib/accountWorkspace";
...
import { CheckCircle2, Database, KeyRound, LogIn, LogOut, ShieldAlert, WifiOff } from "lucide-react";
...
export default function Account() {
  ...
  const onLogin = async (event: FormEvent) => {
    ...
    try {
      ...
      setPassword(""); setAccount(""); setSession(active); await refresh();
      setStatus({ tone: "success", text: `已確認帳號存在並切換至 ${profileLabel(active.profile)}。密碼與登入 token 未寫入本機儲存。` });
    } catch (error) {
      setPassword("");
  ...
  return <div className="account-page">
    <section className="page-titlebar account-titlebar">
      <div><p className="eyebrow">ACCOUNT WORKSPACE / LOCAL ONLY</p><h1>帳號工作區</h1><p>以官方回傳的帳號 ID 分隔本機戰績。排名戰歷史不會在此假設可由帳戶取得；請透過守衛 JSON 或手動建立資料。</p></div>
    ...
    <div className="account-grid">
      <section className="panel account-login-panel">
        ...
        <form className="account-form" onSubmit={onLogin}>
          ...
          <label><span>密碼</span><input type="password" autoComplete="current-password" value={password} onChange={event => setPassword(event.target.value)} placeholder="僅用於本次登入請求" disabled={busy} /></label>
    ...
    <section className="panel account-warning-panel"><header><span><ShieldAlert size={17} /></span><div><p className="eyebrow">READ BEFORE LOGIN</p><h2>重要安全與技術說明</h2></div></header><div className="warning-grid"><div><h3>帳號安全與隱私</h3><p><b>帳號安全風險：</b>在第三方網站輸入密碼可能導致帳號被盜。</p><p><b>隱私風險：</b>您的遊戲資料將在瀏覽器中處理。</p><p><b>ToS 風險：</b>使用非官方工具可能違反遊戲服務條款。</p><p>本網站不會儲存您的密碼，也不會將登入 token 寫入 IndexedDB、localStorage、備份檔或 GitHub；但仍請自行評估風險。</p></div><div><h3>瀏覽器 CORS 限制</h3><p>瀏覽器安全限制、Cloudflare 或網路環境可能使直接連線遊戲伺服器失敗。連線失敗不必然表示帳號或密碼錯誤。</p><p>若失敗，您可以使用「示範模式」檢視功能；或在自行理解風險下，透過瀏覽器開發者工具或官方允許的本地代理測試連線。</p><p>帳戶確認僅建立本機工作區。由於尚未驗證可用的官方排名戰歷史查詢管道，網站不會宣稱或猜測遠端歷史資料。</p></div></div></section>
  </div>;
}
