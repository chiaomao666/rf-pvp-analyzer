...
describe("parsePvpJson", () => {
  it("normalizes a guard-compatible record", () => {
    const parsed = parsePvpJson(JSON.stringify({ records: [{ timestamp: "2026-08-24T22:00:00.000Z", mode: "1v1", result: "victory", playerTeam: [{ name: "我方", level: 80 }], opponentTeam: [{ name: "對方", power: 12345 }], rank_before: 30, rank_after: 22 }] }));
    expect(parsed.rejectedCount).toBe(0);
    expect(parsed.records).toHaveLength(1);
    expect(parsed.records[0]).toMatchObject({ mode: "1v1", outcome: "win", rankBefore: 30, rankAfter: 22 });
  });
  ...
  it("restores portable records into IndexedDB with generated keys", async () => {
    const backup = JSON.stringify({
      ...
      recordCount: 1,
      records: [{ battleAt: 1787603139254, mode: "1v1", outcome: "win", playerTeam: [{ name: "我方" }], opponentTeam: [{ name: "對方" }], sourceBattleChannel: "player:#", sourceBattleId: "fixture-1", createdAt: 1787603139254, updatedAt: 1787603139254 }],
    ...
    await expect(listMatches()).resolves.toMatchObject([{ id: 1, sourceBattleId: "fixture-1", outcome: "win" }]);
