...
(function () {
  ...
  function hasRankingArrays(payload) {
    ...
    const modes = ["1v1", "3v3", "5v5"], response = value.response && typeof value.response === "object" ? value.response : value;
  ...
  function isPvpFrame(frame) {
    ...
    return signature.includes("pvp") || (isPlayerFrame && (location.hash.toLowerCase().includes("/pvpresult") || hasRankingArrays(frame.payload)));
  }
